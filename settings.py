...
DOCUMENTS_DEFAULT = os.path.expanduser(
    os.path.join('~', 'Documents', 'Electronic Arts', 'The Sims 4', 'Mods')
)
mods_folder = DOCUMENTS_DEFAULT

# Location of the game folder
GAME_WINDOWS_DEFAULT = os.path.join('C:', os.sep, 'Program Files', 'EA Games', 'The Sims 4')
game_folder = GAME_WINDOWS_DEFAULT

# The number of threads to use during decompilation
num_threads = 10

# The number of seconds each decompiler will have on each file before being deemed a failure
decompiler_timeout = 180.0

# Instead of compiling everything, should project structure be observed to make sure the code will work the same way as in devmode?
devmode_parity = True
...
debug_cmd_mod_src = "util/debug_cmd.py"
...
decompile_output_folder = os.path.join("..", "EA")
projects_python_path = os.path.join(decompile_output_folder, projects_python_subpath)
projects_tuning_path = os.path.join(decompile_output_folder, projects_tuning_subpath)
